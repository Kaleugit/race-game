...
test('(a) mata-atlantica heightAt matches pre-migration fixture within 1e-9', async () => {
  const registry = await loadStages();
  const stage = registry.getStage('mata-atlantica');
  assert.ok(stage, 'mata-atlantica stage loaded');
